...
class EmbeddingError(ProcessingException):
    ...
    def __init__(self, text_length: int, **kwargs):
        super().__init__(
            message=f"Failed to generate embeddings for text of length {text_length}",
...
class IndexingError(ProcessingException):
    ...
    def __init__(self, index_type: str, **kwargs):
        super().__init__(
            message=f"Failed to index into {index_type}",
...
class OllamaNotAvailableError(LLMException):
    ...
    def __init__(self, base_url: str, **kwargs):
        super().__init__(
            message=f"Ollama service not available at {base_url}",
...
class GenerationTimeoutError(LLMException):
    ...
    def __init__(self, timeout: int, **kwargs):
        super().__init__(
            message=f"LLM generation timed out after {timeout} seconds",
...
def handle_errors(
    ...
    Args:
        error_type: Exception type to catch (default: RAGException)
        log_error: Whether to log the error
        reraise: Whether to reraise after handling
    
    Example:
        @handle_errors(error_type=FileException)
